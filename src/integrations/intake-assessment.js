...
export function assessIntake(item, now = new Date().toISOString(), runtimeConfig = {}) {
...
export function formatIntakeAssessmentComment(assessment) {
  ...
  const automaticWeight = Number(assessment.decisionWeighting?.automaticPercent ?? weighted.automaticPercent ?? 25) / 100;
  const aiWeight = Number(assessment.decisionWeighting?.aiPercent ?? weighted.aiPercent ?? 75) / 100;
  const weightedChecks = assessment.checks.map((check) => {
    const aiReview = ai?.checkReviews?.find((review) => review.checkId === check.id);
    const points = aiReview ? Number(((check.points * automaticWeight) + (aiReview.points * aiWeight)).toFixed(1)) : check.points;
    return `- ${check.label}: ${aiReview ? "weighted" : "automatic"} (${points}/${check.weight})`;
  }).join("\n");
  ...
  return `${INTAKE_MARKER_PREFIX} version=\"${assessment.version}\" -->\nFULCRUM Intake evaluation\nWeighted score: ${weighted.score}/${weighted.maxScore}\nRecommendation: ${weighted.recommendation}\nWeighting: ${weighted.automaticPercent ?? 25}% automatic / ${weighted.aiPercent ?? 75}% AI\n\nWeighted checks:\n${weightedChecks}\n\nOpen items:\n${failures}${aiSection}\n\nFULCRUM_ASSESSMENT_JSON:${JSON.stringify(assessment)}`;
}
