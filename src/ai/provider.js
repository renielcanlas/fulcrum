...
export class OpenAIProvider extends AIProvider {
  ...
  async generateResponse({instructions, input, tools, text, stream = false}) {
    if (!this.apiKey) throw new Error("OPENAI_API_KEY is required for OpenAIProvider");
    const response = await fetch("https://api.openai.com/v1/responses", {method:"POST", headers:{"content-type":"application/json", authorization:`Bearer ${this.apiKey}`}, body:JSON.stringify({model:this.model, instructions, input, tools, text, stream})});
...
export class AzureOpenAIProvider extends AIProvider {
  ...
  async generateResponse({instructions, input, tools, text, stream = false}) {
    ...
    const response = await fetch(`${this.endpoint}/openai/v1/responses`, {method: "POST", headers: {accept: "application/json", "content-type": "application/json", "api-key": this.apiKey}, body: JSON.stringify({model: this.deployment, instructions, input, tools, text, stream})});
    if (!response.ok) {
