...
export function JiraUpdateDialog({issueKey, kind = "description", onConfirm, onCancel}) {
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-[#102f33]/45 p-5" role="presentation">
    <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="ciel-jira-update-title">
      <div className="flex items-start gap-3">
        ...
        <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#087f70]">Jira update</p><h2 id="ciel-jira-update-title" className="mt-1 text-xl font-bold text-[#102f33]">{assignment ? `Reassign ${issueKey}` : transition ? `Change status for ${issueKey}` : `Improve ${issueKey}`}</h2></div>
      </div>
      <p className="mt-5 text-sm leading-6 text-slate-600">{assignment ? "Ciel will resolve the requested synthetic persona from the verified catalog and update this Jira assignee through the FULCRUM service account." : transition ? "Ciel will resolve the requested workflow status and transition this Jira item through the FULCRUM service account." : "Ciel will read the current story, improve its description using the existing facts, and send the updated description to Jira through the FULCRUM service account."}</p>
      <p className="mt-3 rounded-lg bg-slate-50 p-3 text-xs leading-5 text-slate-500">{assignment ? "Only the assignee field will be changed. Status, description, permissions, and decision data will not be modified." : transition ? "With your confirmation, Ciel may update the requested workflow status and, when requested separately, reassign the item to a verified persona or improve its description. Permissions and decision data will not be modified." : "Only the description field will be changed. No status, assignee, permissions, or decision data will be modified."}</p>
