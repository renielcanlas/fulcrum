...
| REQ-017/019 | Read-only Jira attachment preview for Excel/CSV workbooks with worksheet tabs, bounded cell rendering, and original-file download | `app/api/jira/attachment/route.js`, `src/components/spreadsheet-preview.js`, `app/demo/page.js`, `docs/03-architecture/jira-attachment-preview.md` | `test/spreadsheet-preview.test.js`, Excel preview test in `e2e/uat.spec.js`, `npm run build` | Implemented for the synthetic/demo workbench; Jira remains authoritative and workbook write-back is not supported |
