...
**Implemented:** Next.js 16 App Router shell, server Route Handlers for health, sessions, demo users, Ciel, scenario generation, Jira service-account operations, user-authorized Jira comments/attachments, multi-stage evaluation, Azure-compatible AI routing, PDF Document Intelligence extraction, Vercel-compatible build scripts, and Neon-backed persistence for audit events, AI execution records, OAuth state, and encrypted user Jira connections. **Not yet implemented:** durable domain assessment writes, external session store usage for real identities, background queue, and full Jira webhook reconciliation. The synthetic assessment repository remains read-only and the demo session remains intentionally lightweight until the production identity boundary is implemented.
...
1. Create the Next.js application and preserve existing domain modules/tests. **Current status:** complete; the repository has the Next.js App Router shell and route handlers for health, sessions, demo users, Ciel, scenario generation, Jira operations, and stage evaluation.
