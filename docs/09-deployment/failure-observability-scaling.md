...
The natural path is stateless Next.js/Vercel execution, managed PostgreSQL with connection pooling, external session/rate-limit state, and asynchronous workers for Document Intelligence extraction, embeddings, bulk Jira sync, and long-running AI tasks. Control Jira/Azure AI concurrency and rate limits. Do not add Kubernetes or a dedicated gateway for the hackathon.
