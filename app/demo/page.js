      ...
      <div className="space-y-5">
        ...
        {helpTopic === "Evidence & lineage" && <InfoCard title="Evidence & lineage" id="help-evidence-and-lineage">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">When reviewing</p><p className="mt-2 text-sm leading-6 text-slate-600">Start with the source record, check its date and locator, then follow the linked fact to the risk finding and control.</p></div>
            <div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">When challenging</p><p className="mt-2 text-sm leading-6 text-slate-600">Ask which evidence supports the claim, whether anything is stale or missing, and whether the control actually addresses the finding.</p></div>
            <div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">When auditing</p><p className="mt-2 text-sm leading-6 text-slate-600">Use the timestamps, calculation trace, analyst disposition, and committee record to reconstruct what was known at decision time.</p></div>
          </div>
        </InfoCard>}
        {helpTopic === "Human decisions" && <InfoCard title="Human decisions" id="help-human-decisions">
          ...
          <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="font-bold text-amber-950">Typical decision sequence</p><ol className="mt-2 list-decimal space-y-1 pl-5 text-sm leading-6 text-amber-950"><li>Review the deterministic score and its evidence.</li><li>Consider AI observations as challenge and context, not approval.</li><li>Record an authorized human outcome with rationale.</li><li>Track conditions until they are resolved or escalated.</li></ol></div>
          <p className="mt-4 text-sm leading-7 text-slate-600">Product Owners provide context, FCRM Analysts assess and recommend, and Risk Committee members record the final decision. Permissions follow those responsibilities.</p>
        </InfoCard>}
        {helpTopic === "Jira integration" && <InfoCard title="Jira integration" id="help-jira-integration">
          ...
          <h3 className="mt-5 text-sm font-bold text-slate-900">Synthetic demo personas</h3>
          <p className="mt-1 text-sm leading-6 text-slate-600">These are the demo accounts mapped to Jira identities. Use them to validate assignment, permissions, and handoffs in the synthetic FCRM project.</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {[["Maya Chen", "Product Owner"], ["Marcus Thompson", "Product Owner"], ["Daniel Reyes", "FCRM Analyst"], ["Priya Shah", "FCRM Analyst"], ["Helen Morgan", "Risk Committee"], ["Robert Kim", "Risk Committee"]].map(([name, role]) => <div key={name} className="rounded-lg border border-slate-200 bg-slate-50 p-3"><p className="text-sm font-bold text-slate-900">{name}</p><p className="mt-1 text-xs text-slate-500">{role}</p></div>)}
          </div>
          <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4"><p className="font-bold text-blue-950">How to validate Jira integration</p><ol className="mt-2 list-decimal space-y-1 pl-5 text-sm leading-6 text-blue-950"><li>Open the Jira integration view and confirm the connection status.</li><li>Open a synthetic FCRM work item and compare its summary, status, owner, comments, and attachments with Jira.</li><li>Run an evaluation, publish it, then confirm the FULCRUM evaluation comment appears in Jira.</li><li>Move to the next stage and select a different persona; verify both the new assignee and status in Jira.</li><li>Return to FULCRUM and confirm the new stage and evaluation history are visible.</li></ol></div>
        </InfoCard>}
        {helpTopic === "Ciel and AI" && <InfoCard title="Ciel and AI" id="help-ciel-and-ai">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2"><div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">A good Ciel workflow</p><ol className="mt-2 list-decimal space-y-1 pl-5 text-sm leading-6 text-slate-600"><li>Ask for an explanation or missing evidence.</li><li>Check the cited source and current Jira context.</li><li>Review any drafted action.</li><li>Confirm explicitly before a permitted write.</li></ol></div><div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">Signs to pause</p><ul className="mt-2 list-disc space-y-1 pl-5 text-sm leading-6 text-slate-600"><li>The source is missing or stale.</li><li>The answer claims a decision was made.</li><li>The requested action changes scoring or authority.</li><li>The evidence does not support the statement.</li></ul></div></div>
        </InfoCard>}
        {helpTopic === "Risk & controls" && <InfoCard title="Risk & controls" id="help-risk-and-controls">
          ...
          <div className="mt-5 rounded-xl bg-slate-950 p-4 text-sm text-white"><p className="font-bold">Calculation in plain language</p><p className="mt-2 leading-6 text-white/75">FULCRUM starts with the average severity of the risk factors, applies the configured control-mitigation strength, and compares the remaining residual score with S1 and S2. Changing mitigation strength changes the residual number; changing S1/S2 changes the rating band.</p></div>
          <p className="mt-4 text-sm leading-7 text-slate-600">These settings belong to the initiative risk calculation. Jira stage evaluations measure readiness to proceed and use their own stage checks and proceed threshold.</p>
        </InfoCard>}
        {helpTopic === "Sandbox usage" && <InfoCard title="Sandbox usage" id="help-sandbox-usage">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2"><div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">Safe things to test</p><ul className="mt-2 list-disc space-y-1 pl-5 text-sm leading-6 text-slate-600"><li>Search the synthetic FCRM project.</li><li>Draft and validate a bounded action.</li><li>Assign a synthetic persona.</li><li>Add a clearly synthetic comment.</li></ul></div><div className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">Before cleanup</p><p className="mt-2 text-sm leading-6 text-slate-600">Confirm the search is scoped to FCRM and inspect the returned issue list. Cleanup is destructive for those synthetic items and should be used only when intended.</p></div></div>
        ...
        {helpTopic === "Guided demos" && <InfoCard title="Guided demos" id="help-guided-demos">
          ...
          <div className="mt-5 rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">If a demo step does not advance</p><p className="mt-2 text-sm leading-6 text-slate-600">Check that you are on the view named by the step, that the target card is visible, and that the relevant synthetic data has loaded. You can move between steps with Next and restart the tour from Guided demos.</p></div>
