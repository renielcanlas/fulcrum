...
export default function DemoPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/jira?issue=${encodeURIComponent(issueKey)}`)
      .then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "jira_work_item_load_failed"); if (!data.item) throw new Error("work_item_not_found"); setSelectedWorkItem(data.item); const assessmentResponse = await fetch(`/api/jira/assessment?issue=${encodeURIComponent(issueKey)}`); const assessmentData = await assessmentResponse.json(); if (assessmentResponse.ok) setIntakeAssessment(assessmentData); })
  ...
  async function assessIntakeStage() {
    ...
    setAssessmentBusy(true); setAssessmentError(""); setTransitionOffer(false);
    try { const response = await fetch("/api/jira/assessment", {method: "POST", headers: {"content-type": "application/json"}, body: JSON.stringify({action: "assess", issueKey: activeIssueKey})}); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "intake_assessment_failed"); setIntakeAssessment((current) => ({...(current ?? {}), issueKey: activeIssueKey, stage: "Intake", assessment: data.assessment})); }
    catch (error) { setAssessmentError(error.message ?? "intake_assessment_failed"); }
  ...
  async function publishIntakeAssessment() {
    ...
    setAssessmentBusy(true); setAssessmentError("");
    try { const response = await fetch("/api/jira/assessment", {method: "POST", headers: {"content-type": "application/json"}, body: JSON.stringify({action: "publish", issueKey: activeIssueKey, assessment: intakeAssessment.assessment})}); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "intake_assessment_publish_failed"); await refreshWorkItem(activeIssueKey); if (data.assessment.recommendation === "Proceed") setTransitionOffer(true); }
    catch (error) { setAssessmentError(error.message ?? "intake_assessment_publish_failed"); }
    finally { setAssessmentBusy(false); }
  }
  ...
  async function moveToNextStage() {
    ...
    setAssessmentBusy(true); setAssessmentError("");
    try { const response = await fetch("/api/jira/assessment", {method: "POST", headers: {"content-type": "application/json"}, body: JSON.stringify({action: "transition", issueKey: activeIssueKey})}); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "jira_transition_failed"); setTransitionOffer(false); await refreshWorkItem(activeIssueKey); const boardResponse = await fetch("/api/jira"); const boardData = await boardResponse.json(); if (boardResponse.ok) setBoardItems(boardData.items ?? []); }
    catch (error) { setAssessmentError(error.message ?? "jira_transition_failed"); }
    finally { setAssessmentBusy(false); }
  }

  async function refreshWorkItem(issueKey) { const response = await fetch(`/api/jira?issue=${encodeURIComponent(issueKey)}`); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "jira_work_item_load_failed"); setSelectedWorkItem(data.item); const assessmentResponse = await fetch(`/api/jira/assessment?issue=${encodeURIComponent(issueKey)}`); const assessmentData = await assessmentResponse.json(); if (assessmentResponse.ok) setIntakeAssessment(assessmentData); }
    ...
    <main className="min-h-screen bg-[#f5f7f7] text-[rgb(25,66,71)]">
      ...
      <div className="mx-auto flex max-w-[1600px]">
        ...
        <section className="min-w-0 flex-1 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
          ...
          ) : activeView === "work-item" ? <JiraWorkItemView item={selectedWorkItem} userJiraConnected={jiraUserConnected} commentText={commentText} setCommentText={setCommentText} commentBusy={commentBusy} commentError={commentError} onAddComment={addComment} intakeAssessment={intakeAssessment} assessmentBusy={assessmentBusy} assessmentError={assessmentError} transitionOffer={transitionOffer} onAssessIntake={assessIntakeStage} onPublishIntake={publishIntakeAssessment} onMoveToNextStage={moveToNextStage} onDismissTransition={() => setTransitionOffer(false)} onBack={() => { setSelectedWorkItem(null); setActiveView("board"); router.push("/demo"); }} /> : (
            <WorkspaceScreen
              view={activeView}
...
function JiraWorkItemProgress({item}) {
  ...
  return <section className="sticky top-16 z-20 -mt-6 mb-6 -ml-4 -mr-4 border-b border-slate-200 bg-white/95 pb-3 pt-3 shadow-sm backdrop-blur sm:-ml-6 sm:-mr-6 lg:-mt-8 lg:-ml-10 lg:-mr-10" aria-label="Jira work item progress"><div className="mb-3 flex items-end justify-between gap-3 px-4 sm:px-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[rgb(9,167,141)]">Work item progress</p><p className="mt-1 text-sm font-bold text-slate-950">{item?.key ?? "Loading work item…"}</p></div>{item?.url && <a href={item.url} target="_blank" rel="noreferrer" className="shrink-0 text-xs font-semibold text-slate-500 hover:text-[#087f70] hover:underline">Open in Jira ↗</a>}</div><div className="overflow-x-auto pb-1"><div className="flex min-w-[760px] items-stretch">{statuses.map((status, index) => <div key={status} className={`min-w-[150px] flex-1 border-y border-r px-2 py-2 ${currentIndex >= index && currentIndex >= 0 ? "border-[rgba(82,224,129,0.45)] bg-[rgba(82,224,129,0.06)]" : "border-slate-200 bg-white"}`}><div className="flex items-center gap-2"><span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full text-[10px] font-bold ${currentIndex >= index && currentIndex >= 0 ? "bg-[rgb(82,224,129)] text-[rgb(12,34,38)]" : "bg-slate-100 text-slate-400"}`}>{currentIndex >= index && currentIndex >= 0 ? "✓" : index + 1}</span><span className={`text-[10px] font-bold leading-3 ${currentIndex >= index && currentIndex >= 0 ? "text-[rgb(25,66,71)]" : "text-slate-400"}`}>{status}</span></div></div>)}</div></div></section>;
}

function CommentComposer({item, userJiraConnected, jiraUserName, commentText, setCommentText, commentBusy, commentError, onAddComment}) {
  return <div className="mt-8 border-t border-slate-100 pt-6"><h3 className="text-xs font-bold uppercase tracking-wide text-slate-400">Add comment</h3>{userJiraConnected ? <form onSubmit={(event) => {event.preventDefault(); onAddComment();}} className="mt-3"><textarea value={commentText} onChange={(event) => setCommentText(event.target.value)} disabled={commentBusy} className="min-h-28 w-full rounded-lg border border-slate-300 px-3 py-3 text-sm leading-6 outline-none focus:border-[#087f70] focus:ring-2 focus:ring-[#b9e4d1]" placeholder="Write a comment to add to Jira…" /><div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-slate-500">Posting to Jira as <span className="font-semibold text-slate-700">{jiraUserName || "your authorized Jira user"}</span>.</p><button type="submit" disabled={commentBusy || !commentText.trim()} className="rounded-lg bg-[#102f33] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">{commentBusy ? "Posting…" : "Post comment"}</button></div>{commentError && <p className="mt-3 text-xs font-semibold text-red-700" role="alert">{commentError}</p>}</form> : <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-4"><p className="text-sm leading-6 text-slate-600">Connect your Jira account before adding a comment.</p><a href={`/api/jira/user-connect?returnTo=${encodeURIComponent(`/demo?view=work-item&issue=${item.key}`)}`} className="rounded-lg border border-[#087f70] px-4 py-2.5 text-sm font-bold text-[#087f70] hover:bg-[#eef8f2]">Add comment</a></div>}</div>;
}

function IntakeAssessmentPanel({item, intakeAssessment, assessmentBusy, assessmentError, transitionOffer, onAssessIntake, onPublishIntake, onMoveToNextStage, onDismissTransition}) {
  ...
  return <section className="mt-8 rounded-xl border border-[#cfe3d8] bg-[#f7fbf8] p-5" aria-label="FULCRUM Intake assessment"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-[#087f70]">FULCRUM assessment</p><h3 className="mt-1 text-lg font-bold text-[#102f33]">Intake</h3><p className="mt-1 text-sm leading-6 text-slate-600">Check whether this Jira item has enough context to begin assessment.</p></div>{published && !draft && <span className="rounded-full bg-[#dcefe7] px-3 py-1 text-xs font-bold text-[#197443]">Published by Fulcrum</span>}</div>{!assessment ? <button type="button" onClick={onAssessIntake} disabled={assessmentBusy} className="mt-4 rounded-lg bg-[#102f33] px-4 py-2.5 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-40">{assessmentBusy ? "Assessing Intake…" : "Assess Intake"}</button> : <><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Score</p><p className="mt-1 text-2xl font-bold text-[#102f33]">{assessment.score}/{assessment.maxScore}</p></div><div className="rounded-lg bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Recommendation</p><p className="mt-1 text-sm font-bold text-[#197443]">{assessment.recommendation}</p></div><div className="rounded-lg bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Checks</p><p className="mt-1 text-sm font-bold text-[#102f33]">{assessment.checks.filter((check) => check.state === "pass").length}/{assessment.checks.length} passed</p></div></div><ul className="mt-4 space-y-2 text-sm">{assessment.checks.map((check) => <li key={check.id} className="flex gap-2 rounded-lg bg-white px-3 py-2"><span className={check.state === "pass" ? "text-[#197443]" : "text-amber-700"}>{check.state === "pass" ? "✓" : "!"}</span><span><strong className="text-slate-700">{check.label}</strong>{check.failure && <span className="ml-1 text-slate-500">— {check.failure}</span>}</span></li>)}</ul>{history.length > 1 && <div className="mt-4 flex flex-wrap items-center gap-2"><span className="mr-1 text-xs font-bold uppercase tracking-wide text-slate-400">Previous assessments</span>{history.map((version, index) => <button key={version.commentId ?? index} type="button" onClick={() => {setSelectedVersion(index);}} className={`rounded-full px-3 py-1 text-xs font-bold ${index === selectedVersion && !draft ? "bg-[#087f70] text-white" : "bg-white text-[#087f70]"}`}>v{version.revision ?? history.length - index}</button>)}</div>}{draft ? <div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" onClick={onPublishIntake} disabled={assessmentBusy} className="rounded-lg bg-[#102f33] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">{assessmentBusy ? "Publishing…" : "Publish assessment to Jira"}</button><span className="text-xs text-slate-500">Publishing uses Fulcrum’s service account.</span></div> : <div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" onClick={onAssessIntake} disabled={assessmentBusy} className="rounded-lg border border-[#087f70] px-4 py-2.5 text-sm font-bold text-[#087f70] disabled:opacity-40">{assessmentBusy ? "Reassessing…" : "Reassess Intake"}</button><span className="text-xs text-slate-500">A new published version will be added to Jira.</span></div>}</>}{transitionOffer && <div className="mt-4 rounded-lg border border-[#b9e4d1] bg-white p-4 text-sm text-slate-700"><strong>Assessment published.</strong> It recommends proceeding to Context and Research. Would you like to move this Jira item now?<div className="mt-3 flex gap-2"><button type="button" onClick={onMoveToNextStage} disabled={assessmentBusy} className="rounded-lg bg-[#102f33] px-3 py-2 text-xs font-bold text-white">Yes, move it</button><button type="button" onClick={onDismissTransition} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-600">Not now</button></div></div>}{assessmentError && <p className="mt-3 text-xs font-semibold text-red-700" role="alert">{assessmentError}</p>}</section>;
}
...
function PreviousAssessmentSummary({item, intakeAssessment}) {
  ...
  return <section className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4" aria-label="Previous Fulcrum assessments"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Assessment history</p><h3 className="mt-1 text-base font-bold text-[#102f33]">Previous stage summaries</h3></div><span className="text-[11px] text-slate-500">{history.length} published</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{history.map((assessment) => <article key={assessment.commentId ?? `${assessment.stage}-${assessment.publishedAt}`} className="min-w-0 rounded-lg border border-slate-200 bg-white px-2.5 py-2.5"><div className="flex items-center justify-between gap-1"><span className="truncate text-[11px] font-bold text-[#087f70]">{assessment.stage ?? "Assessment"}</span><span className="shrink-0 text-[9px] text-slate-400">{assessment.publishedAt ? new Date(assessment.publishedAt).toLocaleDateString() : ""}</span></div><p className="mt-1 text-sm font-bold text-slate-700">{assessment.score}/{assessment.maxScore}</p><p className="mt-0.5 truncate text-[11px] text-slate-500">{assessment.recommendation ?? "No recommendation"}</p>{assessment.checks?.length > 0 && <ul className="mt-2 space-y-0.5 border-t border-slate-100 pt-1.5">{assessment.checks.map((check) => <li key={check.id} className="flex items-center justify-between gap-1 text-[10px] text-slate-500"><span className="truncate">{check.label}</span><span className={`shrink-0 font-bold ${check.state === "pass" ? "text-[#197443]" : check.state === "partial" ? "text-amber-700" : "text-red-600"}`}>{check.points}/{check.weight}</span></li>)}</ul>}</article>)}</div></section>;
}

function JiraWorkItemView({item, userJiraConnected, commentText, setCommentText, commentBusy, commentError, onAddComment, intakeAssessment, assessmentBusy, assessmentError, transitionOffer, onAssessIntake, onPublishIntake, onMoveToNextStage, onDismissTransition, onBack}) {
