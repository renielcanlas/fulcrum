...
function IntakeAssessmentPanel({item, intakeAssessment, assessmentBusy, assessmentError, transitionOffer, onAssessIntake, onPublishIntake, onMoveToNextStage}) {
  if (item?.statusName !== "Intake") return null;
  const assessment = intakeAssessment?.assessment;
  const published = intakeAssessment?.published;
  return <section className="mt-8 rounded-xl border border-[#cfe3d8] bg-[#f7fbf8] p-5" aria-label="FULCRUM Intake assessment"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-[#087f70]">FULCRUM assessment</p><h3 className="mt-1 text-lg font-bold text-[#102f33]">Intake</h3><p className="mt-1 text-sm leading-6 text-slate-600">Check whether this Jira item has enough context to begin assessment.</p></div>{published && <span className="rounded-full bg-[#dcefe7] px-3 py-1 text-xs font-bold text-[#197443]">Published by Fulcrum</span>}</div>{published ? <div className="mt-4 rounded-lg bg-white p-4 text-sm text-slate-600"><strong className="text-[#102f33]">{published.score}/{published.maxScore}</strong> · {published.recommendation}<p className="mt-1 text-xs text-slate-500">Published to Jira by the FULCRUM service account.</p>{published.recommendation === "Proceed" && <button type="button" onClick={onMoveToNextStage} disabled={assessmentBusy} className="mt-4 rounded-lg bg-[#102f33] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">{assessmentBusy ? "Moving…" : "Move to Context and Research"}</button>}</div> : !assessment ? <button type="button" onClick={onAssessIntake} disabled={assessmentBusy} className="mt-4 rounded-lg bg-[#102f33] px-4 py-2.5 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-40">{assessmentBusy ? "Assessing Intake…" : "Assess Intake"}</button> : <><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Score</p><p className="mt-1 text-2xl font-bold text-[#102f33]">{assessment.score}/{assessment.maxScore}</p></div><div className="rounded-lg bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Recommendation</p><p className="mt-1 text-sm font-bold text-[#197443]">{assessment.recommendation}</p></div><div className="rounded-lg bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Checks</p><p className="mt-1 text-sm font-bold text-[#102f33]">{assessment.checks.filter((check) => check.state === "pass").length}/{assessment.checks.length} passed</p></div></div><ul className="mt-4 space-y-2 text-sm">{assessment.checks.map((check) => <li key={check.id} className="flex gap-2 rounded-lg bg-white px-3 py-2"><span className={check.state === "pass" ? "text-[#197443]" : "text-amber-700"}>{check.state === "pass" ? "✓" : "!"}</span><span><strong className="text-slate-700">{check.label}</strong>{check.failure && <span className="ml-1 text-slate-500">— {check.failure}</span>}</span></li>)}</ul><div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" onClick={onPublishIntake} disabled={assessmentBusy} className="rounded-lg bg-[#102f33] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">{assessmentBusy ? "Publishing…" : "Publish assessment to Jira"}</button><span className="text-xs text-slate-500">Publishing uses Fulcrum’s service account.</span></div></>}{transitionOffer && <div className="mt-4 rounded-lg border border-[#b9e4d1] bg-white p-4 text-sm text-slate-700"><strong>Assessment published.</strong> It recommends proceeding to Context and Research. Would you like to move this Jira item now?<div className="mt-3 flex gap-2"><button type="button" onClick={onMoveToNextStage} disabled={assessmentBusy} className="rounded-lg bg-[#102f33] px-3 py-2 text-xs font-bold text-white">Yes, move it</button><button type="button" className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-600">Not now</button></div></div>}{assessmentError && <p className="mt-3 text-xs font-semibold text-red-700" role="alert">{assessmentError}</p>}</section>;
}

function JiraWorkItemView({item, userJiraConnected, commentText, setCommentText, commentBusy, commentError, onAddComment, intakeAssessment, assessmentBusy, assessmentError, transitionOffer, onAssessIntake, onPublishIntake, onMoveToNextStage, onBack}) {
