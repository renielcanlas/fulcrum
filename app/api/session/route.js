...
export async function POST(request) {
  const previousSid = parseCookie(request.headers.get("cookie") ?? "", cookieName);
  const previousUser = user(request);
  const body = await request.json();
  const selected = findDemoUser(body.userId);
  if (!selected) return Response.json({error:"unknown_or_inactive_demo_user"},{status:400});
