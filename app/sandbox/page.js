...
export default function SandboxPage() {
  ...
  return <main className="min-h-screen bg-[#edf2f0] text-[#172b2f]"><header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-[#d3dfdb] bg-[#102f33] px-5 text-white shadow-lg lg:px-8"><a href="/" className="text-sm font-bold tracking-[0.22em]">FULCRUM <span className="font-normal tracking-normal text-white/60">Sandbox</span></a><div className="flex items-center gap-3"><span className="hidden rounded-full border border-white/20 px-3 py-1.5 text-xs text-white/70 sm:inline">{PROJECT_KEY} project</span><span className={`rounded-lg border px-3 py-2 text-xs font-bold ${connection?.ready ? "border-[#55df82] bg-[#d9f5e1] text-[#197443]" : connection?.authenticated ? "border-amber-300 bg-amber-50 text-amber-800" : "border-white/20 text-white/80"}`}>{connection?.ready ? "✓ Jira connected" : connection?.authenticated ? "⚠ Jira permissions needed" : "Checking Jira..."}</span></div></header><div className="mx-auto flex max-w-[1600px]"><aside className="sticky top-16 hidden h-[calc(100vh-4rem)] w-72 shrink-0 self-start overflow-y-auto border-r border-[#d3dfdb] bg-[#f8faf9] px-4 py-7 lg:block"><p className="px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-[#087f70]">Sandbox</p><p className="mb-7 mt-2 px-3 text-sm font-semibold text-slate-900">All experiments</p><nav className="space-y-1" aria-label="Sandbox navigation">{menu.map(([id, label]) => <button type="button" key={id} onClick={() => setView(id)} className={`w-full rounded-xl px-3 py-3 text-left text-sm font-semibold ${view === id ? "bg-[#dcefe7] text-[#102f33]" : "text-slate-500 hover:bg-white"}`}>{label}</button>)}</nav></aside><div className="min-w-0 flex-1"><nav className="flex gap-2 border-b border-[#d3dfdb] bg-white px-4 py-3 lg:hidden" aria-label="Sandbox navigation">{menu.map(([id, label]) => <button type="button" key={id} onClick={() => setView(id)} className={`rounded-lg px-3 py-2 text-xs font-bold ${view === id ? "bg-[#dcefe7] text-[#102f33]" : "bg-slate-50 text-slate-500"}`}>{label}</button>)}</nav><section className="px-5 py-8 sm:px-8 lg:px-12 lg:py-10">{view === "search" ? <Search jql={jql} setJql={setJql} results={results} onSearch={search} /> : <Automator scenario={scenario} scenarios={scenarios} selectedId={selectedId} setSelectedId={(id) => {setSelectedId(id); setTab("details");}} customJson={customJson} setCustomJson={setCustomJson} tab={tab} setTab={setTab} connection={connection} run={run} onRequestExecute={() => setRun({confirm: true})} onExecute={execute} onCancel={() => setRun(null)} onClose={() => setRun(null)} />}</section></div></div></main>;
}
...
function Automator({scenario, scenarios, selectedId, setSelectedId, customJson, setCustomJson, tab, setTab, connection, run, onRequestExecute, onExecute, onCancel, onClose}) { const confirming = run?.confirm; return <><Header eyebrow="Scenario automator" title="Run a JSON scenario in Jira" text="Select a scenario or paste your own JSON, inspect its plan, then execute each declared step in the fixed FCRM project." /><div className="mt-9 rounded-2xl border border-[#d3dfdb] bg-white shadow-sm"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 p-5"><div><label className="block text-xs font-bold uppercase tracking-[0.14em] text-slate-500" htmlFor="scenario">Scenario source</label><div className="relative mt-2"><select id="scenario" value={selectedId} onChange={(event) => setSelectedId(event.target.value)} className="w-full min-w-64 appearance-none rounded-lg border border-slate-300 bg-white px-3 py-3 pr-11 text-sm font-semibold outline-none focus:border-[#087f70] focus:ring-2 focus:ring-[#b9e4d1]"><option value="" disabled>Select a scenario</option>{scenarios.map((item) => <option key={item.id} value={item.id}>{item.fileName}</option>)}<option value="custom">Custom JSON</option></select><span className="pointer-events-none absolute inset-y-0 right-0 flex w-10 items-center justify-center border-l border-slate-200 text-slate-500" aria-hidden="true">⌄</span></div></div><button type="button" onClick={onRequestExecute} disabled={!scenario || !connection?.connected || run?.active} className="rounded-lg bg-[#102f33] px-5 py-3 text-sm font-bold text-white disabled:opacity-40">Execute scenario</button></div>{selectedId === "custom" && <div className="border-b border-slate-200 px-5 py-5"><label className="block text-xs font-bold uppercase tracking-[0.14em] text-slate-500" htmlFor="custom-json">Paste scenario JSON</label><textarea id="custom-json" value={customJson} onChange={(event) => setCustomJson(event.target.value)} aria-describedby="custom-json-help" className="mt-2 min-h-56 w-full rounded-lg border border-slate-300 px-3 py-3 font-mono text-xs leading-5 outline-none focus:border-[#087f70] focus:ring-2 focus:ring-[#b9e4d1]" placeholder={'{"name":"My scenario","steps":[]}'}/><p id="custom-json-help" className="mt-2 text-xs text-slate-500">Paste an object with a <code>steps</code> array. The project is fixed to FCRM.</p>{customJson.trim() && !scenario && <p className="mt-2 text-xs font-semibold text-red-700" role="alert">Invalid JSON. Check the syntax before continuing.</p>}</div>}{scenario && <><div className="flex border-b border-slate-200 px-5 pt-3"><button type="button" onClick={() => setTab("details")} className={`border-b-2 px-3 py-3 text-sm font-bold ${tab === "details" ? "border-[#087f70] text-[#087f70]" : "border-transparent text-slate-400"}`}>Details</button><button type="button" onClick={() => setTab("json")} className={`border-b-2 px-3 py-3 text-sm font-bold ${tab === "json" ? "border-[#087f70] text-[#087f70]" : "border-transparent text-slate-400"}`}>JSON</button></div>{tab === "details" ? <Details scenario={scenario} /> : <pre className="max-h-[520px] overflow-auto bg-[#102f33] p-6 text-xs leading-6 text-[#d9f4e2]">{JSON.stringify(Object.fromEntries(Object.entries(scenario).filter(([key]) => !["id", "fileName"].includes(key))), null, 2)}</pre>}</>}</div>{confirming && <Confirm scenario={scenario} onCancel={onCancel} onConfirm={() => {onCancel(); onExecute();}} />}{run && !confirming && <Progress run={run} onClose={onClose} />}</>; }
